...
| Routing eval: nothing | Project-level `eval/prompts.jsonl` (20 entries: 12 single / 4 compound / 4 misleading) scored by `eval/run_eval.py`. skill-creator mentions writing test prompts but does not prescribe a per-Skill `evals/` folder, so we use a single project-level surface. |
