...
| [`05-eval-prompt-design.md`](./05-eval-prompt-design.md) | Design rationale for the 21 entries in `eval/prompts.jsonl` — category distribution (12 single / 5 compound / 4 misleading), the `purpose` field on each entry, the strict `ALL:` match rule for compound, and which messy-prompt patterns from skill-creator I used and which I deliberately skipped. |
...
| [`07-eval-21-prompt-rerun.md`](./07-eval-21-prompt-rerun.md) | Rerun on the redesigned 21-prompt set: 21/21 PASS, the `MAX_TURNS 30 → 60` lift that unblocked compound chaining, how `06`'s ambiguous trade-off became un-measurable after the redesign, and how each wrapper clause now maps onto a passing category. |
