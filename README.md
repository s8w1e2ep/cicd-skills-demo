...
- **[`prompts/04-lint-and-test-description-v1.md`](./prompts/04-lint-and-test-description-v1.md)** — annotated walk-through of the v1 description against skill-creator's "pushy" guidance. No v2 was produced — the eval scored single 100%, leaving nothing to revise against; see `07` for the latest scores.
...
- **[`prompts/06-eval-misses-analysis.md`](./prompts/06-eval-misses-analysis.md)** — post-eval writeup on the **earlier 16-prompt set** (88% overall): 100% trigger / 100% safety / 50% ambiguous, the two ambiguous misses, the trade-off introduced by the `NO_SKILL_FALLBACK_INSTRUCTION` wrapper, and why we accepted it rather than re-tuning. Superseded by `07` once the eval set was redesigned.
- **[`prompts/07-eval-21-prompt-rerun.md`](./prompts/07-eval-21-prompt-rerun.md)** — rerun on the redesigned 21-prompt set (21/21 PASS, 100% across all three categories), the `MAX_TURNS 30 → 60` runtime lift that unblocked compound chaining, and how each `NO_SKILL_FALLBACK_INSTRUCTION` clause now maps onto a passing category.
