...
4. **Idempotency check.** If the file existed in step 3:
   ...
   ```bash
   bash .claude/scripts/compare_yaml.sh .github/workflows/dependency-audit.yml /tmp/new-workflow.yml
   ```
...
9. **Read the resulting GitHub Actions runs.**
   ...
   ```bash
   bash .claude/scripts/list_workflow_runs.sh dependency-audit.yml
   ```
